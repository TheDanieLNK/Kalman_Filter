...
% plotting values
figure
plot(X(1,:), 'r')                             % true model internal temperature
hold on
plot(Xh(1,:), 'b')                            % KF internal temperature estimate
hold on
errorbar(1:300,Xh(1,:),sdXH(:), 'b')          % error bars
hold on

ylim([0, 40]);
legend('True Model Internal Temperature', ...
        'Kalman Filter internal Temperature Estimate', ...
